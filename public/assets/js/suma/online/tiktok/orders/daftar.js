var pages = 1;
var status_next_page = paging.status_next_page;
var cursor_next_page = paging.cursor_next_page;
...
function loadMoreDaftarOrders(page = 0, per_page = 10, start_date = '', end_date = '', status = '') {
    if(status_next_page == 1) {
        loading.block();
        $.ajax({
            url: url.daftar_order,
            method: "get",
            data: { page: page + 1, per_page: per_page, start_date: start_date,
                    end_date: end_date, status: status, cursor: cursor_next_page },

            success: function (response) {
                loading.release();
                status_next_page = response.status_next_page;
                cursor_next_page = response.cursor_next_page;
    ...
    $('body').on('click', '#btnRequestPickup', function (e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                loading.block();
                $.ajax({
                    url: url.proses_request_pickup_tiktok,
                    method: "post",
                    data: {
