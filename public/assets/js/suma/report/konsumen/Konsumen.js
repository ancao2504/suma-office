function typemotor() {
    $("#filter_report #tipe_motor").html(
        '<option value="">Pilih Tipe Motor</option>'
    );

    $.each(
        tipemotor[$("#filter_report #merek_motor").val()],
        function (key, value) {
            $("#filter_report #tipe_motor").append(
                `<option value="${value.TypeMotor}">${value.TypeMotor}</option>`
            );
        }
    );
}
...
function request_data(page) {
    ...
    $.ajax({
        ...
        data: {
            ...
            produk:
                $("#filter_report #divisi").val() == "honda"
        ...
        success: function (response) {
            ...
            if ($("#filter_report #lokasi").val() == "") {
                $("#filter_report #lokasi")
                    .val(response.old.request.kd_lokasi)
                    .trigger("change");
            }
...
$("document").ready(function () {
    ...
    $("#filter_report #divisi").on("change", function () {
        $("#filter_report #company").html(
            '<option value="">Pilih Cabang</option>'
        );
        $("#filter_report #lokasi").html(
            '<option value="">Pilih Lokasi</option>'
        );

        if ($(this).val().toLowerCase() == "honda") {
            $(".text-kd_part").text("Kode Part");
            $("#filter_report #kd_part").attr(
                "placeholder",
                "Contoh : 22535KWN901"
            );
            ...
            Object.keys(honda_lokasi).map(function (item) {
                $("#filter_report #company").append(
                    `<option value="${honda_lokasi[item].companyid}">${honda_lokasi[item].companyid} (${honda_lokasi[item].nm_cabang})</option>`
                );
            });
            ...
            Object.keys(fdr_lokasi).map(function (item) {
                $("#filter_report #company").append(
                    `<option value="${fdr_lokasi[item].companyid}">${fdr_lokasi[item].companyid} (${fdr_lokasi[item].nm_cabang})</option>`
                );
            });
    ...
    $("#filter_report #company").on("change", function () {
        if ($(this).val() != "") {
            ...
            lokasi[$("#filter_report #divisi").val()].lokasi[
                $(this).val()
            ].kd_lokasi.forEach(function (item, index) {
                if (index == 0) {
                    $("#filter_report #lokasi").append(
    ...
    $("#filter_report #tgl_lahir1, #tgl_lahir2, #tgl_lahir3, #tgl_lahir4").on(
        "change",
        function () {
            $("#filter_report #tgl_lahir").val("");
        }
    ...
    $("#filter_report #btn-smt").on("click", function () {
        if (
            $("#filter_report #divisi").val() == "" &&
            $("#filter_report #company").val() == ""
        ) {
            Swal.fire({
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        if ($("#filter_report #tgl_lahir1").val() == "") {
                            $("#filter_report #tgl_lahir1")
                                .trigger("click")
                                .focus();
                        } else if (
                            $("#filter_report #tgl_lahir2").val() == ""
                        ) {
                            $("#filter_report #tgl_lahir2")
                                .trigger("click")
                                .focus();
                        } else if (
                            $("#filter_report #tgl_lahir3").val() == ""
                        ) {
                            $("#filter_report #tgl_lahir3")
                                .trigger("click")
                                .focus();
                        } else {
                            $("#filter_report #tgl_lahir4")
                                .trigger("click")
                                .focus();
                        }
                    }
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        if ($("#filter_report #tgl_tran0").val() == "") {
                            $("#filter_report #tgl_tran0")
                                .trigger("click")
                                .focus();
                        } else {
                            $("#filter_report #tgl_tran1")
                                .trigger("click")
                                .focus();
                        }
                    }
    ...
    $("#filter_urutkan #btn-smt").on("click", function () {
        if (
            $("#filter_urutkan #urutkan_collom").val() != "" &&
            $("#filter_urutkan #urutkan").val() != ""
        ) {
            request_data();
                ...
                link.download =
                    ...
                    ($("#filter_report #tgl_lahir").val() != ""
                        ...
                        ? " tgl lahir = " +
                          ...
                          " ,"
                        : "") +
                    ($("#filter_report #jenis_part").val() != ""
                        ? " jenis part = " +
                          $("#filter_report #jenis_part").val() +
                          " ,"
                        : "") +
                    ($("#filter_report #kd_part").val() != ""
                        ? " kode part = " +
                          $("#filter_report #kd_part").val() +
                          " ,"
                        : "") +
                    ($("#filter_report #merek_motor").val() != ""
                        ? " merek motor = " +
                          $("#filter_report #merek_motor").val() +
                          " ,"
                        : "") +
                    ($("#filter_report #tipe_motor").val() != ""
                        ? " tipe motor = " +
                          $("#filter_report #tipe_motor").val() +
                          " ,"
                        : "") +
                    ($("#filter_report #jenis_motor").val() != ""
                        ? " jenis motor = " +
                          $("#filter_report #jenis_motor").val() +
                          " ,"
                        : "") +
