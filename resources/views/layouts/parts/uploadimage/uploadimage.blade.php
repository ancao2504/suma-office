            ...
            <div id="list-gambar" class="row">
                ...
                @if ($dataApi->status == 1)
                    @foreach ($dataApi->data->data as $item)
                        <div class="col-lg-2 col-md-3 col-sm-4 col-lg-3 py-3" onclick="pilihGambar('{{ config('constants.app.url.images').'/parts/'.trim($item->kd_part).'.jpg' }}')" style="cursor: pointer;">
                            <div class="card border border-dark rounded">
                                <div class="d-flex justify-content-center">
                                    @php
                                        $url_img = config('constants.app.url.images').'/parts/'.trim($item->kd_part).'.jpg';
                                        $headers = get_headers($url_img);
                                    @endphp
                                    <div class="bg-image rounded" style="background-image: url('{{ strpos($headers[0], '200')? (config('constants.app.url.images').'/parts/'.trim($item->kd_part).'.jpg') : (config('constants.app.url.images').'/default.png') }}'); width: 100%; height: 200px; background-size: cover; background-position: center; background-repeat: no-repeat;">
