    ...
    <div class="card card-flush mb-6">
        <div class="card-header align-items-center border-0 mt-4 mb-4">
            <h3 class="card-title align-items-start flex-column">
                ...
                <span class="text-muted fw-bold fs-7">Daftar list saldo saya marketplace shopee</span>
                ...
                <table id="tableHistorySaldo" class="table table-row-dashed table-row-gray-300 align-middle">
                    <thead class="border">
                        <tr class="fs-8 fw-bolder text-muted bg-light">
                            ...
                            <th class="w-150px ps-3 pe-3 text-center">Amount</th>
                            <th class="w-150px ps-3 pe-3 text-center">Saldo</th>
                    ...
                    <tbody class="border">
                        ...
                        <tr id="postOrder">
                            <td class="ps-3 pe-3" style="text-align:center;vertical-align:top;
                                @if($data->transaction_type == 'WITHDRAWAL_COMPLETED' || $data->transaction_type == 'WITHDRAWAL_CREATED') background-color: #f1416c;@endif">
                            ...
                            <td class="ps-3 pe-3" style="text-align:right;vertical-align:top;
                                ...
                                <div class="d-flex">
                                    ...
                                    @else
                                        ...
                                        <span class="badge badge-success fs-8 fw-boldest">{{ strtoupper(trim($data->status)) }}</span>
                                        @else
                                        <span class="badge badge-danger fs-8 fw-boldest">{{ strtoupper(trim($data->status)) }}</span>
                                        @endif
                                    @endif
                            ...
                            <td class="ps-3 pe-3" style="text-align:right;vertical-align:top;
                                ...
                                @else
                                    @if((double)$data->amount > 0)
                                    <span class="fs-7 fw-boldest text-success d-block">+ Rp. {{ number_format($data->amount) }}</span>
                                    @else
                                    <span class="fs-7 fw-boldest text-danger d-block">- Rp. {{ number_format($data->amount) }}</span>
                                    @endif
                            ...
                            <td class="ps-3 pe-3" style="text-align:right;vertical-align:top;
                                ...
                                <span class="fs-7 fw-bolder d-block
                                    ...
                                    @else
                                        text-dark
                                    @endif">Rp. {{ number_format($data->current_balance) }}
                                </span>
                            </td>
                            <td class="ps-3 pe-3" style="text-align:center;vertical-align:top;
                                @if($data->transaction_type == 'WITHDRAWAL_COMPLETED' || $data->transaction_type == 'WITHDRAWAL_CREATED') background-color: #f1416c;@endif">
...
<script>
    const url = {
        'daftar_history_saldo_group': "{{ route('online.historysaldo.shopee.daftar-group') }}",
        'detail_history_saldo_group': "{{ route('online.historysaldo.shopee.daftar-group-detail') }}",
    }
    const data = {
...
<script src="{{ asset('assets/js/suma/online/shopee/historysaldo/daftar-group.js') }}?v={{ time() }}"></script>
@endpush
@endsection
