...
@if($data->faktur->status == 1)
    @if((double)$data->faktur->total_amount != (double)$data->tiktok->item_price)
...
@foreach($data->faktur->list as $faktur)
    @if(strtoupper(trim($faktur->ekspedisi->kode)) == 'TKPDEXP')
    <div class="alert alert-dismissible bg-danger d-flex flex-column flex-sm-row w-100 p-5">
        <span class="svg-icon svg-icon-2hx svg-icon-light me-4 mb-5 mb-sm-0">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                <path opacity="0.3" d="M12 22C13.6569 22 15 20.6569 15 19C15 17.3431 13.6569 16 12 16C10.3431 16 9 17.3431 9 19C9 20.6569 10.3431 22 12 22Z" fill="currentColor"></path>
        ...
        <div class="d-flex flex-column text-light pe-0 pe-sm-10">
            <h4 class="mb-2 text-light">Informasi</h4>
            <span>Data kurir recommendasi masih belum di validasi</span>
        </div>
    </div>
    @endif
@endforeach
    ...
    <div class="card card-flush shadow">
        <div class="card-header align-items-center border-0 mt-4">
            <h3 class="card-title align-items-start flex-column">
                ...
                <span class="text-muted fw-bold fs-7">Data invoice marketplace tiktok</span>
            </h3>
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-lg-6 col-md-6 col-sm-6 pt-6 pb-6">
                    ...
                    <span class="fs-7 fw-bolder text-dark d-block mt-1">{{ $data->tiktok->nomor_invoice }}</span>
                    <span class="fs-7 fw-bolder text-primary d-block">{{ $data->tiktok->order_id }}</span>
                    ...
                    <div class="fw-bolder fs-7 text-gray-800 d-flex align-items-center flex-wrap mt-1">
                        <span class="pe-2">{{ $data->tiktok->logistic->name }}</span>
                        <span class="fs-7 text-danger d-flex align-items-center">
                        <span class="bullet bullet-dot bg-danger me-2"></span>{{ $data->tiktok->logistic->service }}</span>
                    </div>
                    ...
                    <div class="d-flex flex-stack w-200px mt-1">
                        ...
                        <div class="fs-7 fw-boldest text-danger text-end">Rp. {{ number_format($data->tiktok->item_price) }}</div>
                    </div>
                    <div class="d-flex flex-stack w-200px mt-1">
                        <div class="fs-7 fw-bolder text-gray-800">Shipping:</div>
                        <div class="fs-7 fw-bolder text-dark text-end">Rp. {{ number_format($data->tiktok->shipping_price) }}</div>
                    </div>
                    ...
                    <div class="d-flex flex-stack w-250px mt-1">
                        ...
                        <div class="fs-7 fw-bolder text-danger text-end">{{ $data->tiktok->shipment_fulfillment->accept_deadline }}</div>
                    </div>
                    <div class="d-flex flex-stack w-250px mt-1">
                        <div class="fs-7 fw-bolder text-gray-800">Shipping:</div>
                        <div class="fs-7 fw-bolder text-danger text-end">{{ $data->tiktok->shipment_fulfillment->confirm_shipping_deadline }}</div>
                    </div>
                </div>
                <div class="col-lg-6 col-md-6 col-sm-6 pt-6 pb-6 ps-6">
                    <span class="fs-7 fw-bolder text-gray-500 d-block">Payment:</span>
                    <span class="fs-7 fw-bolder text-gray-800 d-block mt-1">{{ $data->tiktok->payment->ref_number }}</span>
                    <span class="fs-7 fw-bolder text-gray-800 d-block">{{ date('d F Y', strtotime($data->tiktok->payment->date)) }}</span>
                    @if($data->tiktok->payment->status == 'verified')
                    ...
                    <span class="fs-8 fw-boldest badge badge-light-danger mt-2">
                        <i class="fa fa-times me-2 text-danger" aria-hidden="true"></i>{{ strtoupper($data->tiktok->payment->status) }}
                    </span>
                    @endif

                    <span class="fs-7 fw-bolder text-gray-500 d-block mt-6">Recipient:</span>
                    <span class="fs-7 fw-bolder text-dark d-block mt-1">{{ $data->tiktok->address->district }}</span>
                    <span class="fs-7 fw-bolder text-gray-800 d-block">{{ $data->tiktok->address->city }}, {{ $data->tiktok->address->province }}</span>
                    <span class="fs-7 fw-bolder text-gray-800 d-block">{{ $data->tiktok->address->postal }}</span>

                    <span class="fs-7 fw-bolder text-gray-500 d-block mt-6">Status:</span>
                    <span class="fs-8 fw-boldest badge @if($data->tiktok->status->is_accepted == true) badge-light-success @else badge-light-danger @endif me-2 mt-2">
                        @if($data->tiktok->status->is_accepted == true)
                    ...
                    <span class="fs-8 fw-boldest badge @if($data->tiktok->status->is_confirm_shipping == true) badge-light-success @else badge-light-danger @endif me-2 mt-2">
                        @if($data->tiktok->status->is_confirm_shipping == true)
                    ...
                    <span class="fs-8 fw-boldest badge @if($data->tiktok->status->is_item_delivered == true) badge-light-success @else badge-light-danger @endif me-2 mt-2">
                        @if($data->tiktok->status->is_item_delivered == true)
                    ...
                    @foreach ($data->tiktok->voucher as $voucher)
            ...
            <div class="row">
                <div class="table-responsive">
                    <table class="table table-row-dashed table-row-gray-300 align-middle">
                        ...
                        <tbody class="border">
                            @forelse($data->tiktok->detail as $detail_tiktok)
                            <tr>
                                ...
                                <td class="ps-3 pe-3" style="text-align:left;vertical-align:top;">
                                    <div class="d-flex">
                                        <div class="symbol symbol-45px me-5">
                                            <img src="{{ $detail_tiktok->pictures }}" alt="{{ $detail_tiktok->sku }}">
                                        </div>
                                        <div class="d-flex justify-content-start flex-column">
                                            <span class="fs-7 fw-bolder text-dark">{{ $detail_tiktok->product_name }}</span>
                                            <span class="fs-8 fw-bolder text-gray-600 d-block">(SKU : {{ $detail_tiktok->sku }})</span>
                                        </div>
                                    </div>
                                </td>
                                <td class="ps-3 pe-3" style="text-align:right;vertical-align:top;">
                                    <span class="fs-7 fw-bolder text-gray-800">{{ number_format($detail_tiktok->quantity) }}</span>
                                </td>
                                <td class="ps-3 pe-3" style="text-align:right;vertical-align:top;">
                                    <span class="fs-7 fw-bolder text-gray-800">{{ number_format($detail_tiktok->product_price) }}</span>
                                </td>
                                <td class="ps-3 pe-3" style="text-align:right;vertical-align:top;">
                                    <span class="fs-7 fw-bolder text-gray-800">{{ number_format($detail_tiktok->subtotal_price) }}</span>
                                </td>
                            </tr>
                            @empty
                            <tr>
    ...
    @else
        ...
        <div class="card card-flush shadow mt-6">
            ...
            <div class="card-body">
                ...
                <div class="row">
                    @if (strtoupper(trim($data_internal->ekspedisi->kode)) == 'TKPDEXP')
                    <div class="d-flex">
                        <button id="btnUpdateKurir" name="update_kurir" type="button" class="btn btn-sm btn-danger"
                            data-nomor_invoice="{{ trim($data->tiktok->nomor_invoice) }}">
                            <i class="fa fa-truck text-white" data-toggle="tooltip" data-placement="top" title="Select"></i> Validasi Kurir
                        </button>
                    </div>
                    @endif
...
<script>
    const url = {
        ...
        'proses_order': "{{ route('online.orders.tiktok.form.proses') }}",
        ...
        'proses_update_kurir': "{{ route('online.orders.tiktok.form.update-kurir') }}",
    }
    const data = {
        'nomor_invoice': "{{ $data->tiktok->nomor_invoice }}",
        'tanggal': "{{ $tanggal }}",
    }
</script>
<script src="{{ asset('assets/js/suma/online/tiktok/orders/form.js') }}?v={{ time() }}"></script>
@endpush
@endsection
