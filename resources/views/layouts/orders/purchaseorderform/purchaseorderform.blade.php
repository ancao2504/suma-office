...
<div class="row g-0">
    <div class="card card-flush">
        <div class="card-header align-items-center border-0 mt-4 mb-4">
            <h3 class="card-title align-items-start flex-column">
                ...
                <span class="text-muted fw-bold fs-7">Daftar purchase order form
                ...
                @if(trim($data_filter->kode_sales) != '' || trim($data_filter->kode_dealer) != '')
            ...
            <div class="card-toolbar">
                <button id="btnFilterMasterData" class="btn btn-primary" type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalFilter">
                ...
                <table class="table table-row-dashed table-row-gray-300 align-middle">
                    <thead class="border">
                        <tr class="fs-8 fw-boldest text-muted">
                            <th class="w-50px text-center ps-3 pe-3">No</th>
                            <th class="w-100px text-center ps-3 pe-3">Nomor POF</th>
                            <th class="w-50px text-center ps-3 pe-3">Sales</th>
                            <th class="w-50px text-center ps-3 pe-3">Dealer</th>
                            <th class="min-w-100px text-center ps-3 pe-3">Keterangan</th>
                            <th class="w-50px text-center ps-3 pe-3">TPC</th>
                            <th class="w-50px text-center ps-3 pe-3">TOP</th>
                            <th class="w-75px text-center ps-3 pe-3">Status</th>
                            <th class="w-100px text-center ps-3 pe-3">Order</th>
                            <th class="w-100px text-center ps-3 pe-3">Terlayani</th>
                            <th class="w-50px text-center ps-3 pe-3">BO</th>
                            <th class="w-50px text-center ps-3 pe-3">Action</th>
                        </tr>
                    </thead>
                    <tbody class="border">
                        ...
                        <tr>
                            ...
                            <td class="ps-3 pe-3" style="text-align:left;vertical-align:top;">
                                <div class="row align-items-start">
                                    <a href="{{ route('orders.purchaseorderform.form.detail', trim($data->nomor_pof)) }}" class="fs-7 fw-boldest text-gray-800 text-hover-primary">{{ trim($data->nomor_pof) }}</a>
                                    <span class="fs-7 fw-bolder text-gray-700">{{ date('d F Y', strtotime($data->tanggal)) }}</span>
                                    <span class="fs-7 fw-bolder text-muted">{{ trim($data->jam) }}</span>
                                </div>
                                <div class="row align-items-end">
                                    <div class="mt-6"></div>
                                </div>
                                <div class="row align-items-end">
                                    @if(trim($data->no_order) != '')
                                    <span class="fs-7 fw-bolder text-gray-400">Nomor Order:</span>
                                    <span class="fs-7 fw-bolder text-gray-800">{{ trim($data->no_order) }}</span>
                                    @endif
                                    <div class="d-flex mt-4">
                                        @if((int)$data->pmo == 1)
                                        <span class="badge badge-light-danger fs-8 fw-boldest">MOBILE</span>
                                        @elseif((int)$data->pmo_web == 1)
                                        <span class="badge badge-light-info fs-8 fw-boldest">WEB</span>
                                        @else
                                        <span class="badge badge-light-success fs-8 fw-boldest">DESKTOP</span>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="ps-3 pe-3" style="text-align:center;vertical-align:top;">
                                <span class="fs-8 fw-boldest text-primary">{{ $data->umur_pof }}</span>
                            </td>
                            <td class="ps-3 pe-3" style="text-align:center;vertical-align:top;">
                                @if((int)$data->status_faktur == 1)
                                <div class="fs-8 fw-boldest text-success">TERPROSES</div>
                                @else
                                    @if((int)$data->approve == 1)
                                    <div class="fs-8 fw-boldest text-info">APPROVED</div>
                                    @else
                                    <div class="fs-8 fw-boldest text-danger">ON PROGRESS</div>
                                    @endif
                                @endif
                            </td>
                            <td class="ps-3 pe-3" style="text-align:center;vertical-align:top;">
                                <span class="fs-8 fw-boldest text-gray-800">{{ number_format($data->total_order) }}
                                    <span class="fs-8 fw-boldest text-gray-600 ms-1">PCS</span>
                                </span>
                            </td>
                            <td class="ps-3 pe-3" style="text-align:center;vertical-align:top;">
                                @if((double)$data->total_terlayani >= (double)$data->total_order)
                                <span class="fs-8 fw-boldest text-success">{{ number_format($data->total_terlayani) }}
                                    <span class="fs-8 fw-boldest text-gray-600 ms-1">PCS</span>
                                </span>
                                @elseif((double)$data->total_terlayani > 0)
                                <span class="fs-8 fw-boldest text-primary">{{ number_format($data->total_terlayani) }}
                                    <span class="fs-8 fw-boldest text-gray-600 ms-1">PCS</span>
                                </span>
                                @else
                                <span class="fs-8 fw-boldest text-gray-800">{{ number_format($data->total_terlayani) }}
                                    <span class="fs-8 fw-boldest text-gray-600 ms-1">PCS</span>
                                </span>
                                @endif
                            </td>
                            <td class="ps-3 pe-3" style="text-align:center;vertical-align:top;">
                                @if(strtoupper(trim($data->bo)) == 'B')
                                <div class="fs-8 fw-boldest text-danger">BO</div>
                                @else
                                <div class="fs-8 fw-boldest text-warning">TIDAK BO</div>
                                @endif
                            </td>
                            <td class="ps-3 pe-3" style="text-align:center;vertical-align:top;">
                                <a href="{{ route('orders.purchaseorderform.form.detail', trim($data->nomor_pof)) }}" class="btn btn-icon btn-primary btn-sm">
    ...
    <div class="card card-flush mt-6">
        <div class="card-body ribbon ribbon-top ribbon-vertical pt-5">
            ...
            @endif
            <div class="row mt-4">
                <span class="fw-bold fs-7 text-gray-600">Nomor POF:</span>
                <span class="fw-bolder text-dark mt-1">{{ trim($data->nomor_pof) }}</span>
            </div>
            <div class="row mt-6">
                <span class="fw-bold fs-7 text-gray-600">Tanggal POF:</span>
                <span class="fw-bolder text-dark mt-1">{{ date('j F Y', strtotime($data->tanggal)) }}</span>
                <span class="fw-bolder text-gray-600 mt-1">{{ trim($data->jam) }}</span>
            </div>
            <div class="row mt-6">
            ...
            <div class="row">
                <div class="col-6 mt-6">
                    <div class="row">
                        ...
                        @if(strtoupper(trim($data->bo)) == 'B')
                        <div class="fs-7 fw-boldest text-danger">BO</div>
                        @else
                        <div class="fs-7 fw-boldest text-warning">TIDAK BO</div>
                        @endif
                    </div>
                </div>
                <div class="col-6 mt-6">
                    ...
                    <div class="d-flex align-items-center flex-wrap mt-1">
                        <span class="fs-7 fw-bolder text-dark d-flex align-items-center">{{ trim($data->umur_pof) }} Hari
                            <span class="bullet bullet-dot bg-danger ms-2 me-2"></span>
                            <span class="fw-bolder text-danger">{{ $data->tanggal_akhir_pof }}</span>
                        </span>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-6 mt-6">
                    <div class="row">
                        <span class="fw-bold fs-7 text-gray-600">Device:</span>
                        <div class="d-flex">
                            @if((int)$data->pmo == 1)
                            <span class="fs-7 fw-boldest text-danger">MOBILE</span>
                            @elseif((int)$data->pmo_web == 1)
                            <span class="fs-7 fw-boldest text-info">WEB</span>
                            @else
                            <span class="fs-7 fw-boldest text-success">DESKTOP</span>
                            @endif
                        </div>
                    </div>
                </div>
                <div class="col-6 mt-6">
                    <div class="row">
                        <span class="fw-bold fs-7 text-gray-600">Status:</span>
                        @if((int)$data->status_faktur == 1)
                        <div class="fs-7 fw-boldest text-success">TERPROSES</div>
                        @else
                            @if((int)$data->approve == 1)
                            <div class="fs-7 fw-boldest text-info">APPROVED</div>
                            @else
                            <div class="fs-7 fw-boldest text-danger">ON PROGRESS</div>
                            @endif
                        @endif
                    </div>
                </div>
            ...
            <div class="row">
                <div class="col-6 mt-6">
                    <div class="row">
                        <span class="fw-bold fs-7 text-gray-600">Order:</span>
                        <span class="fs-7 fw-boldest text-gray-800">{{ number_format($data->total_order) }}
                            <span class="fs-8 fw-boldest text-gray-600 ms-1">PCS</span>
                        </span>
                    </div>
                </div>
                <div class="col-6 mt-6">
                    <div class="row">
                        <span class="fw-bold fs-7 text-gray-600">Terlayani:</span>
                        @if((double)$data->total_terlayani >= (double)$data->total_order)
                        <span class="fs-7 fw-boldest text-success">{{ number_format($data->total_terlayani) }}
                            <span class="fs-8 fw-boldest text-gray-600 ms-1">PCS</span>
                        </span>
                        @elseif((double)$data->total_terlayani > 0)
                        <span class="fs-7 fw-boldest text-primary">{{ number_format($data->total_terlayani) }}
                            <span class="fs-8 fw-boldest text-gray-600 ms-1">PCS</span>
                        </span>
                        @else
                        <span class="fs-7 fw-boldest text-gray-800">{{ number_format($data->total_terlayani) }}
                            <span class="fs-8 fw-boldest text-gray-600 ms-1">PCS</span>
                        </span>
                        @endif
                    </div>
            ...
            <a href="{{ route('orders.purchaseorderform.form.detail', trim($data->nomor_pof)) }}" class="btn btn-primary mb-2" id="viewPof" role="button">
                <i class="fa fa-check text-white" data-toggle="tooltip" data-placement="top" title="Select"></i> Lihat Purchase Order
            </a>
        </div>
    </div>
    @empty
    ...
    <div class="modal-dialog">
        <div class="modal-content" id="modalFilterContent">
            ...
            <div class="modal-footer justify-content-between">
                ...
                <div class="text-end">
                    <button id="btnFilterProses" type="submit" role="button" class="btn btn-primary">Terapkan</button>
...
@push('scripts')
    ...
    <script src="{{ asset('assets/js/suma/orders/purchaseorderform/purchaseorderform.js') }}?v={{ time() }}"></script>
@endpush
@endsection
