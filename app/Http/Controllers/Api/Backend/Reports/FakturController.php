        ...
        try {
            ...
            $data = DB::table('faktur')
                ->select(
                    ...
                    // DB::raw(
                    //     'iif(
                    //                 isnull(faktur_total.faktur_total, 0) <= 0,
                    //                     isnull(faktur.total, 0),
                    //                     isnull(faktur.total, 0) - round(((isnull(faktur.DiscRp, 0) / isnull(faktur_total.faktur_total, 0))), 0) - round(((isnull(faktur.DiscRp1, 0) / isnull(faktur_total.faktur_total, 0))),0)
                    //                 ) as total'
                    // )
                )
                ->from(function ($query) use ($request) {
                    $query->select(
                        ...
                        'fakt_dtl.jml_order',
                        'fakt_dtl.jml_jual',
                        DB::raw('isnull(fakt_dtl.jumlah, 0) - round((isnull(fakt_dtl.jumlah, 0) * isnull(faktur.disc2, 0)) / 100, 0) - isnull(fakt_dtl.discrp1_total, 0) - isnull(fakt_dtl.discrp1_sisa, 0) as total')
                    )
                        ->from(function ($query) use ($request) {
                            $query->select(
                                'faktur.companyid',
                ...
                //         'faktur.companyid',
                //         'faktur.no_faktur',
                //         DB::raw('count(fakt_dtl.no_faktur) as faktur_total')
                //     )
                //         ->from(function ($query) use ($request) {
                //             $query->select(
                //                 'faktur.companyid',
        ...
        try {
            ...
            $data = DB::table('faktur')
                ->select(
                    ...
                    'faktur.jml_jual',
                    'faktur.total',
                    // DB::raw('iif(isnull(faktur_total.faktur_total, 0) <= 0,isnull(faktur.total, 0),isnull(faktur.total, 0) - round(((isnull(faktur.DiscRp, 0) / isnull(faktur_total.faktur_total, 0))), 0) - round(((isnull(faktur.DiscRp1, 0) / isnull(faktur_total.faktur_total, 0))), 0)) as total')
                )
                ->from(function ($query) use ($request) {
                    $query->select(
                        'faktur.companyid',
