...
class PurchaseOrderController extends Controller
{
    public function index(Request $request)
    {
        ...
        $responseApi = ApiService::PurchaseOrderFormDaftar(
            $year,
            $month,
            $kode_sales,
            $kode_dealer,
        ...
        if ($statusApi == 1) {
            ...
            if ($Agent->isMobile()) {
                ...
                $data_pof = new LengthAwarePaginator(
    ...
    {
        $responseApi = ApiService::PurchaseOrderFormTransfer(
            $nomor_pof,
            strtoupper(trim($request->session()->get('app_user_id'))),
            strtoupper(trim($request->session()->get('app_user_role_id'))),
            strtoupper(trim($request->session()->get('app_user_company_id')))
        ...
        if ($statusApi == 1) {
            $responseApi = ApiService::PurchaseOrderFormDetail(
                $nomor_pof,
                strtoupper(trim($request->session()->get('app_user_id'))),
                strtoupper(trim($request->session()->get('app_user_company_id')))
            );
            ...
            if ($statusApi == 1) {
                ...
                return view('layouts.orders.purchaseorder.purchaseorderform', [
                    ...
                    'total_terlayani' => (float)$data->total_terlayani,
    ...
    {
        $responseApi = ApiService::PurchaseOrderFormUpdateTpc(
            $request->get('nomor_pof'),
            $request->get('tpc'),
            strtoupper(trim($request->session()->get('app_user_id'))),
            strtoupper(trim($request->session()->get('app_user_company_id')))
        );
    ...
    {
        $responseApi = ApiService::purchaseOrderFormEditDiscount(
            $request->get('nomor_pof'),
            strtoupper(trim($request->session()->get('app_user_id'))),
            strtoupper(trim($request->session()->get('app_user_company_id')))
        );
    ...
    {
        $responseApi = ApiService::purchaseOrderFormUpdateDiscount(
            $request->get('nomor_pof'),
            (float)$request->get('discount'),
            strtoupper(trim($request->session()->get('app_user_id'))),
            strtoupper(trim($request->session()->get('app_user_company_id')))
        );
    ...
    {
        $responseApi = ApiService::PurchaseOrderFormDetailEditPart(
            $request->get('nomor_pof'),
    ...
    {
        $responseApi = ApiService::PurchaseOrderFormDetailSimpanPart(
            $request->get('nomor_pof'),
            $request->get('part_number'),
            (float)$request->get('jml_order'),
            (float)$request->get('harga'),
            (float)$request->get('discount'),
            strtoupper(trim($request->session()->get('app_user_id'))),
            strtoupper(trim($request->session()->get('app_user_company_id')))
        );
    ...
    {
        $responseApi = ApiService::PurchaseOrderFormDetailHapusPart(
            $request->get('nomor_pof'),
            $request->get('part_number'),
            strtoupper(trim($request->session()->get('app_user_id'))),
            strtoupper(trim($request->session()->get('app_user_company_id')))
