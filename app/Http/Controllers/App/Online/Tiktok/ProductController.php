...
class ProductController extends Controller
{
    public function index(Request $request) {
        ...
        return view ('layouts.online.tiktok.product.product', [
            'title_menu'    => 'Products',
    ...
    public function daftarPartNumber(Request $request) {
        $responseApi = ServiceTiktok::ProductSearchPartNumber(strtoupper(trim($request->get('part_number'))),
                ...
                <table class="table align-middle gs-0 gy-3">
                    <thead class="border">
                        <tr class="fs-8 fw-bolder text-muted">
                            ...
                            <th class="min-w-300px ps-3 pe-3 text-center">Tiktok</th>
    ...
    public function cekProductId(Request $request) {
        $responseApi = ServiceTiktok::ProductCekProductId(strtoupper(trim($request->get('product_id'))),
                        strtoupper(trim($request->session()->get('app_user_company_id'))));
        $statusApi = json_decode($responseApi)->status;
        ...
        if($statusApi == 1) {
            ...
            if($dataApi->internal->status == 0) {
                $data_tiktok = '<div class="fv-row mt-6">
            ...
            return ['status' => 1, 'message' => 'success', 'data' => $data_tiktok];
        } else {
            return json_decode($responseApi, true);
        }
    }
    ...
    public function updateProductId(Request $request) {
        $responseApi = ServiceTiktok::ProductUpdateProductId(strtoupper(trim($request->get('part_number'))),
